...
function writeAnswer(buffer, offset, name, answer) {
  ...
  switch (
    ...
    case TYPE_AAAA:
      try {
        const ipv6 = ipaddr.IPv6.parse(answer.data);
        const bytes = ipv6.toByteArray();
        for (let i = 0; i < 16; i++) {
          buffer.writeUInt8(bytes[i], dataOffset + i);
        }
      } catch (err) {
        console.error(`Error parsing IPv6 address ${answer.data}:`, err);
        // Fallback to zeros if invalid
        for (let i = 0; i < 16; i++) {
          buffer.writeUInt8(0, dataOffset + i);
        }
      }
      length = 16;
      break;
