...
function parseDomainName(buffer, offset) {
  ...
  while (true) {
    if (jumps > MAX_JUMPS) {
      throw new Error("DNS parse error: excessive pointer jumps (possible infinite loop)");
    }
...
function createResponse(query, answers) {
  ...
  buffer.writeUInt16BE(0, 10); // ARCountt
  offset = 12;
  ...
  // Returning the complete DNS packet buffer 
}
